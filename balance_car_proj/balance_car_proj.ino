...
void Angle_Calcu(void)
{
  ...
  Angle_aYZ = atan2((Out_Data(ACCEL_YOUT_H) - 300),
                      (Out_Data(ACCEL_ZOUT_H) - (16384 - 16450))) * 180 / PI; //去除零點偏移,計算得到角度（弧度），並把角度(弧度)轉換為度,

  //-------角速度-------------------------
  //不自測，陀螺儀測量範圍設置  0  ±250°/s  131LSB/(°/s)   0.00763358 (°/s)/LSB
  Angle_gX = (Out_Data(GYRO_XOUT_H) - 0) * 0.00763358; //0為補償量，在靜止是測得的角速度為0LSB(老色逼)；
...
void loop()
{
  ...
  mpu.getEvent(&a, &g, &temp);//(a=加速度、g=傾角、temp=溫度)拿溫度到底要衝三小
